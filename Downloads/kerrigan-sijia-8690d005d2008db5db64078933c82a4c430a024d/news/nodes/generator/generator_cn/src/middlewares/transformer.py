...
class Transformer(MapMiddleware):
    ...
    def transform(self, item):
        if item['need_upload']:
            ...
            pb = pbFactory.getPB(self.get_resource_file_path, page_type)
            html = pb.build(item)
            return html
        else:
            return None
...
class PBFactory(object):
    ...
    def getPB(self, get_resource_file_path, hint):
        if hint == 'album':
            return PB_Album(self.settings, get_resource_file_path, hint)
        elif hint == 'video':
            return PB_Video(self.settings, get_resource_file_path, hint)
        elif hint == 'tencent_open':
            return PB_TencentNews(self.settings, get_resource_file_path, hint)
        else:
            return PB_Article(self.settings, get_resource_file_path, hint)


class PB_Video(object):

    def __init__(self, settings, get_resource_file_path, hint):
        self.settings = settings
        self.pb_type = hint
        self.default_share_icon = self.settings['DEFAULT_SHARE_ICON']
        self.pattern_html = open(
            get_resource_file_path('video_share_article.html')).read()
...
class PB_TencentNews(object):
    ...
    def __init__(self, settings, get_resource_file_path, hint):
        ...
        self.pb_type = hint
        self.pattern_html = open(
            get_resource_file_path('tencent_open_article.html')).read()
...
class PB_Article(object):
    ...
    def __init__(self, settings, get_resource_file_path, hint):
        ...
        self.pb_type = hint
        self.pattern_html = open(
            get_resource_file_path('base_article_v1.html')).read()
...
class PB_Album(object):
    ...
    def __init__(self, settings, get_resource_file_path, hint):
        ...
        self.pb_type = hint
        self.pattern_html = open(
            get_resource_file_path('base_album.html')).read()
