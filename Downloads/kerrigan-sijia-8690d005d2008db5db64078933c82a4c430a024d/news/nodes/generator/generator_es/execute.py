...
class Generator(object):
    ...
    def process_throughout(self, message):
        ...
        if json.loads(message)['process_mode'] != 'sim_mode':
            if json.loads(message)['account'].split('-')[0] in ['46', '48']:
                logging.info(
                    "tops found")
                logging.info(
                    {json.loads(message)['source_url']: json.loads(message)['dedup_key']})
                with open('/pixdata/data/news/resource/url_ctid', 'a') as f:
                    f.write(str({json.loads(message)['source_url']: json.loads(message)['dedup_key']}) + '\n')
        return message
