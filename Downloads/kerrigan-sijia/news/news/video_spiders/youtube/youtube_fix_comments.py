...
class YoutubeSpider(YoutubeSpiderBase):
    ...
    def __init__(self, *a, **kw):
        ...
        self.logger.info(len(doc_list))
        for i in self.doc_set.find({"source_name": "youtube"}):
            hash_value = settings.getint("hash_value", default=-1)
            hash_total = settings.getint("hash_total", default=-1)
            if i['doc_id'] in doc_url_list:
                continue
    ...
    def parse_page(self, response):
        ...
        comment_list = self.get_comment_list(video_id)
        ...
        if comment_list:
            ...
            # def start_requests(self):
            #     raw = self.content_list.pop(0)
            #     video_id = raw['source_url'].split('v=')[-1]
            #     comment_list = YoutubeStatistic.get_comment_list(video_id=video_id)
            #     for each in comment_list:
            #         publisher_hash = int(hashlib.md5(str(each['publisher'])).hexdigest(), 16)
            #         user_id = self.fake_users[publisher_hash % len(self.fake_users)]
            #         payload = {'comment_content': each['text']}
            #         target_url = 'https://www.veeuapp.com/internal/docs/%s/comments?_user_id=%s' % (raw['doc_id'], user_id)
