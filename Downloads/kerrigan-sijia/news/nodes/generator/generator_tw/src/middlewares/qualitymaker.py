...
class QualityMaker(MapMiddleware):
    ...
    def process(self, item):
        path = "feeds/generator/generator_tw/resources/" if self.settings.get('TEST_MODE', False) else "resources/"
        with open(os.path.join(path, 'quality/weixin1'), 'r') as f:
            for line in f:
                source_list_2.add(line.replace('\n', '').decode('utf-8'))

        with open(os.path.join(path, 'quality/weixin2'), 'r') as f:
            for line in f:
                source_list_3.add(line.replace('\n', '').decode('utf-8'))

        with open(os.path.join(path, 'quality/toutiao1'), 'r') as f:
            for line in f:
                source_list_4.add(line.replace('\n', '').decode('utf-8'))

        with open(os.path.join(path, 'quality/toutiao2'), 'r') as f:
            for line in f:
