...
public class PeerConnectionClient {
  ...
  private CameraVideoCapturer videoCapturer2;
  // enableVideo is set to true if video should be rendered and sent.
  private boolean renderVideo;
  private VideoTrack localVideoTrack;
  private VideoTrack remoteVideoTrack;
  private VideoTrack localVideoTrack2;
  ...
  private void createCapturer(CameraEnumerator enumerator) {
    ...
    for (String deviceName : deviceNames) {
      if (enumerator.isFrontFacing(deviceName)) {
        Logging.d(TAG, "Creating front facing camera capturer.");
        videoCapturer2 = enumerator.createCapturer(deviceName, null);
  ...
  private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
    ...
    if (videoCallEnabled) {
      ...
      mediaStream.addTrack(createVideoTrack(videoCapturer2));
    }
    ...
    executor.execute(new Runnable() {
      ...
      public void run() {
        ...
        if (localVideoTrack2 != null) {
          localVideoTrack2.setEnabled(renderVideo);
        }
      }
    });
  }
  ...
  public void stopVideoSource() {
    executor.execute(new Runnable() {
      ...
      public void run() {
        ...
        if (videoCapturer2 != null && !videoCapturerStopped) {
          Log.d(TAG, "Stop video source.");
          try {
            videoCapturer2.stopCapture();
          } catch (InterruptedException e) {}
  ...
  /*private VideoTrack createVideoTrack2(VideoCapturer capturer2) {
    videoSource2 = factory.createVideoSource(capturer2);
    capturer2.startCapture(videoWidth, videoHeight, videoFps);

    localVideoTrack2 = factory.createVideoTrack(VIDEO_TRACK_ID, videoSource);
    localVideoTrack2.setEnabled(renderVideo);
    localVideoTrack2.addRenderer(new VideoRenderer(localRender2));
...
/*
  public void onCameraDualInternal() {
    if (!videoCallEnabled || numberOfCameras < 2 || isError || videoCapturer == null) {
      Log.e(TAG, "Failed to enable both camera. Video: " + videoCallEnabled + ". Error : "
              + isError + ". Number of cameras: " + numberOfCameras);
      return;  // No video is sent or only one camera is available or error happened.
    }
    Log.d(TAG, "Dual camera");
    videoCapturer.dual(null);
  }
  */
    ...
    public void onAddStream(final MediaStream stream){
      executor.execute(new Runnable() {
        ...
        public void run() {
          ...
          if (stream.audioTracks.size() > 1 || stream.videoTracks.size() > 3) {
            reportError("Weird-looking stream: " + stream);
            return;
          }
          if (stream.videoTracks.size() == 2) {
